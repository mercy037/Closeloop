...
def train_agent_model_free_multi_task_car_following(agent, envs, params):
    ...
    cumulative_timestep, n_updates, i_episode, samples_number, task_number = load_checkpoint_multi_task_car_following(
        agent, 'Dec16_16-26-06', 'test1215-4750')
    n_updates = 0
    samples_number = 0
    ...
    while True:
        ...
        i_episode += 1
        if i_episode % 250 == 0:  # Condition to terminate, also let's cache agent and envs for plotting later
            if save_model:
                current_time = datetime.now().strftime('%b%d_%H-%M-%S')
        ...
        while not done:
            ...
            if i_episode < 100:
                ...
                for i, val in enumerate(next_state):
                    if state_max[i] - state_min[i] == 0:
                        next_state[i] = next_state[i]
                    else:
                        next_state[i] = (val - state_max[i]) / (state_max[i] - state_min[i])


            writer.add_scalar('Info/num_episode', info["num_episode"], cumulative_timestep)
            writer.add_scalar('Info/碰撞率', info["collision_rate"], cumulative_timestep)
            writer.add_scalar('Info/滑动窗口平均碰撞率', info["window_collision_rate"], cumulative_timestep)
            writer.add_scalar('Info/本回合奖励', info["episode_total_reward"], cumulative_timestep)
            writer.add_scalar('Info/截至目前平均奖励', info["average_reward"], cumulative_timestep)
            writer.add_scalar('Info/滑动窗口平均奖励', info["window_average_reward"], cumulative_timestep)
            ...
            if samples_number > n_collect_steps:
                alpha_, actor_loss, critic_loss = agent.optimise()
                n_updates += 1
                writer.add_scalar('Info/actor_loss', actor_loss, cumulative_timestep)
                writer.add_scalar('Info/critic_loss', critic_loss, cumulative_timestep)
            if (time_step >= 1000):
                done = True
            if done:
...
def main():
    ...
    owl_parser.add_argument('--n_collect_steps', type=int, default=256)
    ...
    if owl_args.owl:
        agent = SACAgentOWL(state_dim, action_dim, max_action, min_action, num_tasks=owl_args.num_tasks,
                            ...
                            gamma=0.99, q_lr=3e-4, batch_size=256, tau=0.005, adaptive_alpha=True, pool_size=1000000)
    else:
        agent = SACAgent(state_dim=3, action_dim=1, max_action=3.0, min_action=-3.0, num_tasks=1, hidden_size=256, actor_lr=0.0003,
                         critic_lr=0.0003, q_ewc_reg=owl_params['q_ewc_reg'], gamma=0.99, q_lr=3e-4, batch_size=256, tau=0.005, adaptive_alpha=True, pool_size=1000000)
