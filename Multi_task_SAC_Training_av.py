...
def make_env(env_id):
    ...
    if env_id == 1:
        carla_gym_parser.add_argument('--cfg_file', type=str, default='./tools/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=25100, type=int,
                                      help='TCP port to listen to (default: 5100)')
        carla_gym_parser.add_argument('--tm_port', default=36100, type=int,
        ...
        carla_gym_parser.add_argument('--cfg_file', type=str, default='./tools/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=25400, type=int,
                                      help='TCP port to listen to (default: 5400)')
        carla_gym_parser.add_argument('--tm_port', default=36400, type=int,
                                      help='Traffic Manager TCP port to listen to (default: 16400)')

    elif env_id == 6:
        carla_gym_parser.add_argument('--cfg_file', type=str, default='./tools/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=6600, type=int,
                                      help='TCP port to listen to (default: 5400)')
        carla_gym_parser.add_argument('--tm_port', default=16600, type=int,
                                      help='Traffic Manager TCP port to listen to (default: 16400)')

    elif env_id == 7:
        carla_gym_parser.add_argument('--cfg_file', type=str, default='./tools/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=15700, type=int,
                                      help='TCP port to listen to (default: 5400)')
        carla_gym_parser.add_argument('--tm_port', default=26700, type=int,
                                      help='Traffic Manager TCP port to listen to (default: 16400)')

    elif env_id == 8:
        carla_gym_parser.add_argument('--cfg_file', type=str, default='./tools/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=15800, type=int,
                                      help='TCP port to listen to (default: 5400)')
        carla_gym_parser.add_argument('--tm_port', default=26800, type=int,
                                      help='Traffic Manager TCP port to listen to (default: 16400)')

    elif env_id == 10:
        carla_gym_parser.add_argument('--cfg_file', type=str, default='./tools/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=25110, type=int,
                                      help='TCP port to listen to (default: 5400)')
        carla_gym_parser.add_argument('--tm_port', default=36110, type=int,
                                      help='Traffic Manager TCP port to listen to (default: 16400)')
    ...
    if env_id == 1:
        ...
        print('Env6 is starting')
        env = gym.make('gym_env-v6')
    elif env_id == 7:
...
if __name__ == '__main__':
    ...
    owl_parser.add_argument('--max_task_frames', type=int, default=500000)
    ...
    if train_npc == 1:
        model = CustomSAC("CustomSACPolicy", env=env_multi[0], policy_kwargs=policy_kwargs,
                          ...
                          tensorboard_log="./Training_Results/0521_fisher/Tensorboard_Results/", verbose=1)
        checkpoint_callback = CheckpointCallback(save_freq=250000,
                                                 save_path='./Training_Results/Normal_CustomSAC_75/AV_Model_SAC', verbose=1)
        # create a SwitchEnvCallback object with the list of environments and the switch frequency
        switch_env_callback = SwitchEnvCallback(env_multi, switch_freq=[1500000, 750000, 750000])
        ...
        model.learn(total_timesteps=9000000, log_interval=1, callback=callback_list, tb_log_name="train_Normal_CustomSAC_75",
                    reset_num_timesteps=False)
        model.save("./Training_Results/Normal_CustomSAC_75/AV_Model_SAC_final")

    # 2.加载已有模型，在其基础上进行训练
    if train_npc == 2:
        model = CustomSAC.load("./Training_Results/0317_5/AV_Model_SAC/rl_model_2000000_steps.zip", env=env_multi[1],
                               device=device)
        q_ewc_reg = 0.000      # 0.0001 15   0.001 16  0.01 17 0.1 18 失败 0.05 19
        ...
        checkpoint_callback = CheckpointCallback(save_freq=250000,
                                                 save_path='./Training_Results/0317_5/AV_Model_SAC', verbose=1)
        switch_env_callback = SwitchEnvCallback(env_multi, switch_freq=500000)
        switch_env_callback.num_steps = model.num_timesteps-1000
        switch_env_callback.env_index = 1
        switch_env_callback.task_idx = 1
        callback_list = CallbackList([checkpoint_callback, switch_env_callback])
        model.learn(total_timesteps=9000000, log_interval=1, callback=callback_list, tb_log_name="train_19", reset_num_timesteps=False)
        model.save("/data/csy/Training_Results/0321_4/AV_Model_SAC_final")
    # 0. 加载已有模型，进行测试
    elif train_npc == 0:
        total_reward = 0
        step = 0
        model = CustomSAC.load("./Training_Results/0317/AV_Model_SAC/rl_model_1550000_steps.zip", env=env_multi[1])
        task_idx = 0
