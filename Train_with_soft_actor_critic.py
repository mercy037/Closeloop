...
class Memory():
    ...
    def store_transition(self,s,a,r,s_):
        tran = np.hstack((s, [a[0],a[1],r], s_))  # 把s,a,r,s_困在一起，水平拼接
...
if __name__ == '__main__':
    ...
    action_number = env.action_space.shape[1] # 在对抗场景的环境中该数值为2,动态修改
    ...
    if Switch == 0:
        ...
        for episode in range(EP_MAX):
            ...
            #observation = np.array(observation).reshape(30,1)
            ...
            # if reward_total > -10: RENDER = True
            all_ep_r.append(reward_total)
            if episode % 20 == 0 and episode > 60:#保存神经网络参数
                save_data = {'net': actor.action_net.state_dict(), 'opt': actor.optimizer.state_dict(), 'i': episode}
                torch.save(save_data, './Training_Results/1002/')
        env.close()
