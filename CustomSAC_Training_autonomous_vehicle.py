...
def parse_args_cfgs():
    ...
    parser.add_argument('-p', '--carla_port', metavar='P', default=5500, type=int,
                        help='TCP port to listen to (default: 2000)')
    parser.add_argument('--tm_port', default=5040, type=int,
...
if __name__ == '__main__':
    ...
    if train_npc == 1:
        model = CustomSAC("CustomSACPolicy", env=env, num_tasks=num_tasks, q_ewc_reg=q_ewc_reg,
                          ...
                          tensorboard_log="./Training_Results/0125/Tensorboard_Results/", verbose=1)
        checkpoint_callback = CheckpointCallback(save_freq=250000,
                                                 save_path='./Training_Results/0125/AV_Model_SAC', verbose=1)
        model.learn(total_timesteps=3700000, log_interval=1, callback=checkpoint_callback, reset_num_timesteps=False)
        model.save("./Training_Results/0125/AV_Model_SAC_final")
   ...
   # 0. 加载已有模型，进行测试
        ...
        total_reward = 0
        step = 0
        model = CustomSAC.load("./Training_Results/0321/AV_Model_SAC/rl_model_1000000_steps.zip", env=env)
        task_idx = 0
