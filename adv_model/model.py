...
class Encoder(torch.nn.Module):
    def __init__(self, n_head, in_dim, out_dim):
        ...
        self.layer_norm_1 = torch.nn.LayerNorm(30)
...
class Activation_Net(torch.nn.Module):
    ...
    def __init__(self, in_dim=6, n_hidden_1=512, n_hidden_2=512, out_dim=2): #######
...
class Transformer(torch.nn.Module):
    def __init__(self, n_head):
        ...
        self.encoder = Encoder(n_head, in_dim=1, out_dim=1)  # 37是human readable日期格式的字符串vocabulary数，和onehot向量长度一直
        self.decoder = Decoder(n_head, in_dim=1, out_dim=1)  # 12是machine readable日期格式的字符串vocabulary数，和onehot向量长度一直
        self.linear = Activation_Net()
...
if __name__ == '__main__':
    ...
    batch_x = torch.randn(1, 31, 1)  # (N, T, in_dim)
    batch_y = torch.randn(1, 1, 1)  # (N, T, in_dim)
