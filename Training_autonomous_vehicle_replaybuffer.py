...
if __name__ == '__main__':
    ...
    # 1.生成一个新的模型并进行训练
    if train_npc == 1:
        model = CustomSAC("MlpPolicy", env=env,  tensorboard_log="./Training_Results/save_replay_buffer/Tensorboard_Results_80_20_1/", verbose = 1)
        checkpoint_callback = CheckpointCallback(save_freq=100000, save_path='./Training_Results/save_replay_buffer/AV_Model_SAC_80_20_1',save_replay_buffer=True,verbose = 1)
        model.learn(total_timesteps=1200000, log_interval=1, callback = checkpoint_callback, reset_num_timesteps = False)
        model.save("./Training_Results/save_replay_buffer/AV_Model_SAC_80_20_1")


    # 2.加载已有模型，在其基础上进行训练
    if train_npc == 2:
        model = CustomSAC.load("./Training_Results/save_replay_buffer/AV_Model_SAC_90_10/rl_model_200000_steps.zip" , env=env, device = device)
        checkpoint_callback = CheckpointCallback(save_freq=100000, save_path='./Training_Results/save_replay_buffer/AV_Model_SAC_90_10_4',save_replay_buffer=True, verbose = 1)
        model.learn(total_timesteps=1000000, log_interval=1, callback=checkpoint_callback, reset_num_timesteps=False)
        model.save("./Training_Results/save_replay_buffer/AV_Model_SAC_90_10_4")

   # 0. 加载已有模型，进行测试
    elif train_npc == 0:
        total_reward = 0
        step = 0
        model = SAC.load("./Training_Results/save_replay_buffer/AV_Model_SAC_90_10_4/rl_model_900000_steps.zip", env=env)
        obs = env.reset() #初始化环境
        data = []
        episode = 0
        try:
