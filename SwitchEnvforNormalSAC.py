...
class SwitchEnvCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self.model.logger.record("train/episode_total_reward", self.model.env.envs[0].episode_total_reward)
        self.model.logger.record("train/average_reward", self.model.env.envs[0].average_reward)
        self.model.logger.record("train/window_average_reward", self.model.env.envs[0].window_average_reward)
        self.model.logger.record("train/collision_rate", self.model.env.envs[0].collision_rate)
        self.model.logger.record("train/window_collision_rate", self.model.env.envs[0].window_collision_rate)
        self.model.logger.record("train/num_episode", self.model.env.envs[0].num_episode)

        # check if it is time to switch the environment
        # print("current env:", self.env_index)
        if self.num_steps % self.switch_freq == 0:
            # # 冻结训练完成任务的策略头网络
