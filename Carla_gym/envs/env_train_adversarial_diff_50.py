...
from agents.local_planner.frenet_optimal_trajectory_adv import FrenetPlanner as MotionPlanner2
...
import heapq
import torch
from adv_model.model import Transformer
from adv_model.util import save_load as sl
...
def update_adversarial_bv(actors_batch, x_av, y_av, idx, flag_adv_lanechange):
    ...
    if update == True:
        ...
        flag_adv_lanechange = False

    # 如果更新标志为假，将上一个时刻的对抗车辆作为下一个时刻的对抗车辆
    else:
        idx_new = idx

    return idx_new, update, flag_adv_lanechange
...
class CarlagymEnv8(gym.Env):
    ...
    def __init__(self):
        ...
        '''Autonomous Driving'''
        self.obs_av_dim = (
        1, 30)  # [ego_s, ego_d, v_s, v_d, phi_frenet, obj_s, obj_d, obj_v_s, obj_v_d, obj_phi_frenet]
        ...
        # 对抗相关
        self.flag_adv_lanechange = False
        self.diff = 1.0
    ...
    def BV_action_to_control(self, action, ego_s, adbv_s):

        '''Initial Flags'''
        self.f_idx = 1
        loop_counter = 0
        vf_action = np.clip(action[0], -1.0, 1.0) * (19 - 7) / 2 + (19 + 7) / 2
        d_action = np.clip(action[1], -1.0, 1.0) * (0.25 - (-1)) / 2 + (0.25 + (-1)) / 2
        print(f"vf_action: {vf_action}, d_action: {d_action}")
        ''' Action Design '''
        # if action_bv.ndim == 2:
        ...
        delta_s = adbv_s - ego_s
        if delta_s >= 7.3:  # 达到切入条件
            df_n = d_action
            self.flag_adv_lanechange = True
        if self.flag_adv_lanechange:
            df_n = d_action
        else:
            wp = self.world_module.town_map.get_waypoint(adbv.get_location())
            lane_id = wp.lane_id
            df_n = lane_id * 3.5
        ...
        acc = math.sqrt(acc_vec.x ** 2 + acc_vec.y ** 2 + acc_vec.z ** 2)
        acc_abs = math.sqrt(acc_vec.x ** 2 + acc_vec.y ** 2)
        ...
        fpath, self.lanechange_adbv, off_the_road = self.motionPlanner2.run_step_single_path(adbv_state, self.f_idx,
                                                                                             df_n,
                                                                                             Tf=1.2,
                                                                                             Vf_n=vf_action)  # 根据当前车辆状态进行路径规划，自车的状态
    ...
    def step(self, action):
        ...
        self.idx_bv, update, self.flag_adv_lanechange = \
            update_adversarial_bv(self.traffic_module.actors_batch, self.ego.get_location().x,
                                  self.ego.get_location().y, self.idx_bv,
                                  self.flag_adv_lanechange)
        ...
        # model_bv = SAC.load("./Training_Results/0417_bv/BV_Model_SAC/rl_model_100000_steps.zip")
        # action_bv, _ = model_bv.predict(self.obs_bv)    #predict输出action和state,state表示模型的内部状态，若没有则为None

        '''载入模型预测'''
        model_adv = Transformer(n_head=2)
        model_path = 'adv_model/experiment_2'
        model_adv = sl.load_model(model_path, -1, model_adv)
        model_adv.eval()

        # 难度系数
        model_adv_input = np.concatenate((self.obs_adv, [self.diff]))
        model_adv_input = torch.Tensor(model_adv_input).float().unsqueeze(dim=-1).unsqueeze(dim=0)
        pred = model_adv(model_adv_input)
        action_adv = [0, 0]
        action_adv[0] = float(pred[0][0])
        action_adv[1] = float(pred[0][1])
        ...
        if self.idx_bv != -1:
            ...
            try:
                control_bv = self.BV_action_to_control(action_adv, ego_s, adbv_s)
            except RuntimeError:
                adbv = self.traffic_module.actors_batch[self.idx_bv]['Actor']
                self.vehicleController2 = VehiclePIDController(adbv,
                                                               args_longitudinal={'K_P': 40.0, 'K_D': 0.1, 'K_I': 4.0},
        ...
        self.obs_adv = self.state_adv(v_S, ego_s, ego_d, current_acc)
        ...
        reward = float(
            reward_cl + reward_sp + reward_distance + reward_overtake +
            reward_comfort + reward_not_go)  # + reward_alive_distance)
    ...
    def state_adv(self, v_S, ego_s, ego_d, current_acc):
        # Paper: Automated Speed and Lane Change Decision Making using Deep Reinforcement Learning
        obj_mat = self.obj_info()
        adv_state_vector = np.zeros(5)

        # No normalized
        adv_state_vector[0] = obj_mat[0][self.idx_bv] - ego_s
        adv_state_vector[1] = obj_mat[2][self.idx_bv] - v_S
        adv_state_vector[2] = v_S
        adv_state_vector[3] = current_acc
        adv_state_vector[4] = obj_mat[1][self.idx_bv] - ego_d

        return adv_state_vector
