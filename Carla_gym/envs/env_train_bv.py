...
class CarlagymEnv0(gym.Env):
    ...
    def __init__(self):
       ...
       # self.lon_param = MPC_lon_Config
        ...
        self.action_dim_av = (1, 2)
        """连续动作空间"""
        self.action1_low = -1.0 # action的范围为-1到1，可以在输出的基础上乘以一个系数来限制范围
        self.action1_high = 1.0
        # 此处定义
        self.action_space = spaces.Box(low = self.action1_low, high = self.action1_high, shape = (2,), dtype = 'float32')
    ...
    def step(self, action):
        ...
        if self.idx_bv != -1:
            reward = reward_adv + collision_loss + distance_reward + fail_loss
        else:
            reward = 0
