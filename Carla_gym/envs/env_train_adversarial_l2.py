...
def update_adversarial_bv(actors_batch, x_av, y_av, idx, flag_adv_active):
    ...
    if update == True:
        ...
        adv_active = random.random()
        if adv_active > 0.6:
            flag_adv_active = False
        else:
            flag_adv_active = True

    # 如果更新标志为假，将上一个时刻的对抗车辆作为下一个时刻的对抗车辆
    else:
        idx_new = idx

    return idx_new, update, flag_adv_active
...
class CarlagymEnv5(gym.Env):
    ...
    def AV_action_to_control(self, action_av):
        ...
        if action_av.ndim == 2:
            print("df:{},a:{},v:{},idx_bv:{},active:{}".format(action_av[0][0], action_av[0][1],
                                                               self.v_S, self.idx_bv, self.flag_adv_active))
            df_n = action_av[0][0] * 3.5
            acc_input = np.clip(action_av[0][1] * 8.0, -8.0, 2.0)
        else:
            print("df:{},a:{},v:{},idx_bv:{},active:{}".format(action_av[0], action_av[1],
                                                               self.v_S, self.idx_bv, self.flag_adv_active))
    ...
    def step(self, action):
        ...
        self.idx_bv, update, self.flag_adv_active = update_adversarial_bv(self.traffic_module.actors_batch,
                                                                          self.ego.get_location().x,
                                                                          self.ego.get_location().y,
                                                                          self.idx_bv,
                                                                          self.flag_adv_active)
        if self.flag_adv_active == False:
            active_idx_bv = -1
        else:
            active_idx_bv = self.idx_bv
        ...
        if update == True and active_idx_bv != -1:
            adbv = self.traffic_module.actors_batch[self.idx_bv]['Actor']
        ...
        if self.n_step == 1:  # 第一步，更新30步，让所有车辆进入高速行驶状态
            ...
            new_action = [active_idx_bv, control_bv]
