...
class CustomReplayBuffer(ReplayBuffer):
    ...
    def sample_fisher(self, batch_size: int, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        """
        Sample the last `batch_size` experiences from the replay buffer.
        """
        print("success enter sample_fisher")
        print(f"buffer_size: {self.buffer_size}")
        # Get the indices of the last `batch_size` experiences
        last_inds = np.arange(self.pos - batch_size, self.pos)
        return self._get_samples(last_inds, env=env)
...
class CustomSAC(SAC):
    ...
    def get_sample(self, sample_size):
        ...
        replay_data = self.replay_buffer.sample_fisher(sample_size, env=self._vec_normalize_env)
