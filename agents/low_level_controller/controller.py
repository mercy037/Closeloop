...
class VehiclePIDController:
    ...
    def run_step_2_wp(self, target_speed, waypoint1, waypoint2):
        ...
        :return: distance (in meters) to the waypoint
        """
        throttle, brake, speed = self._lon_controller.run_step_new(target_speed)
...
class PIDLongitudinalController:
    ...
    def run_step_new(self,target_speed):
        current_speed = get_speed(self._vehicle)
        if self._pid_control_new(target_speed, current_speed)>= 0:
            throttle = self._pid_control_new(target_speed, current_speed)
            brake = 0.0
        else:
            brake = -self._pid_control_new(target_speed, current_speed)
            throttle = 0.0
    ...
    def _pid_control_new(self, target_speed, current_speed):
        """
        Estimate the throttle of the vehicle based on the PID equations
