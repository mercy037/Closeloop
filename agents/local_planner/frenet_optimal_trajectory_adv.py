...
class FrenetPlanner:
    ...
    def run_step_single_path(self, ego_state, idx, df_n=0, Tf=4, Vf_n=0):
        ...
        f_state = self.estimate_frenet_state(ego_state, idx)
        # if df_n < -0.33:        # convert lateral action value from range (-1, 1) to the desired value in [-3.5, 0.0, 3.0, 7.0]
        #     df = -1
        # elif df_n > 0.33:
        #     df = 1
        # else:
        ...
        lanechange = True if abs(df_n - d) >= 3 else False
        ...
        Vf = Vf_n
        # Frenet motion planning
        self.path = self.generate_single_frenet_path(f_state, Vf=Vf, df=df_n, Tf=Tf)
