...
class FrenetPlanner:
    ...
    def estimate_frenet_state(self, ego_state, idx):
        ...
        f_state = [self.path.s[idx], self.path.s_d[idx], self.path.s_dd[idx],#f_state为期望路径
        ...
        estimated_s = self.path.s[idx] % ego_state[6]#除以总路程后的余数，path为规划出的路径
        estimated_s -= update_s(estimated_s)#估计当前状态和期望状态的误差
    ...
    def calc_frenet_paths(self, f_state, change_lane=0, target_speed=30 / 3.6):
        ...
        #限制侧向位置的数值，本程序不换道，即目标d处于-3.5到7之间
        frenet_paths = []
    ...
    def frenet_optimal_planning(self, f_state, change_lane=0, target_speed=30 / 3.6):
        ...
        fplist = self.calc_frenet_paths(f_state, change_lane=change_lane, target_speed=target_speed)#根据当前状态生成候选路径
        fplist = self.calc_global_paths(fplist)# 坐标系转换
        fplist = self.calc_curvature_paths(fplist) #坐标系转换
        fplist = self.check_paths(fplist) # 去除发生碰撞的路径
    ...
    def reset(self, s, d, df_n=0, Tf=4, Vf_n=0, optimal_path=True):
        ...
        if optimal_path:#计算最优路径
            best_path_idx, fplist = self.frenet_optimal_planning(f_state) #规划出最优路径（考虑碰撞和价值最大化）
            self.path = fplist[best_path_idx]#path存储的是当前状态下的最优路径
        else:
