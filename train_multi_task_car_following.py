...
def train_agent_model_free_multi_task_car_following(agent, envs, params):
    ...
    while True:
        ...
        if i_task != i_tasks_prev:
            ...
            '''
            if save_model:
                make_checkpoint_multi_task_car_following(agent, cumulative_timestep, tag,
        ...
        while not done:
            ...
            samples_number += 1
            state = next_state
            if samples_number > n_collect_steps:
            ...
            if done:
                ...
                print("episode:{} mean episode score:{} episode time steps:{}".format(i_episode, mean_episode_score,
                                                                                      info[1]))
            ...
            if cumulative_timestep % log_interval == 0:
                ...
                episodes_return_mean, episode_return, = evaluate_agent_oracle(envs, agent, episode_max_steps,
                ...
                if bandit_eval:
                    for i in range(num_tasks):
                        ...
                        writer.add_scalar('Reward/BNDT_Returns_unmean_{}'.format(i), Returns_bandit_unmean,
                                          cumulative_timestep)
                ...
                with np.printoptions(precision=2, suppress=True):
                    print(
                        'Task {}, Episode {}, Samples {}, Train RPE: {:.3f}, Number of Policy Updates: {}, Cumulative time steps: {}'.format(
                            task_number,
...
def make_env(env_id):
    ...
    if env_id == 1:
        carla_gym_parser.add_argument('--cfg_file', type=str, default='tools_env_1/cfgs/config.yaml',
                                      help='specify the config for training')
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=5300, type=int,
                                      help='TCP port to listen to (default: 2000)')
        carla_gym_parser.add_argument('--tm_port', default=16299, type=int,
...
def main():
    ...
    owl_parser.add_argument('--n_collect_steps', type=int, default=5000)
    owl_parser.add_argument('--env', dest='env', default='multi_task_car_following', type=str,
                            help='The multi-task car following env.')
    owl_parser.add_argument('--save_model', dest='save_model', action='store_true', default=True)
    owl_parser.add_argument('--tag', type=str, default='sac_ewc_car_following_multi_env', help='unique str to tag tb.')
    owl_parser.add_argument('--logdir', dest='logdir', default='runs_multi_task_car_following_multi_env', type=str,
                            help='The log directory for tensorboard.')
    # CL params
    owl_parser.add_argument('--num_tasks', type=int, default=3)
    owl_parser.add_argument('--num_task_repeats', type=int, default=1)
    owl_parser.add_argument('--max_task_frames', type=int, default=50000)
    owl_parser.add_argument('--q_ewc_reg', dest='q_ewc_reg', type=float, default=10000,
    ...
    owl_parser.add_argument('--owl', dest='owl', default=False, action='store_true',
    ...
    if owl_args.env == 'multi_task_car_following':
        ...
        for env_id in env_ids:
            ...
            max_action = 3
            min_action = -3
            print("Env{}: state_dim: {}, action_dim: {}，max_action: {}, min_action: {}.".format(env_id, state_dim,
    ...
    if owl_args.owl:
        agent = SACAgentOWL(state_dim=3, action_dim=1, max_action=3.0, min_action=-3.0, num_tasks=owl_args.num_tasks,
                            hidden_size=256, actor_lr=0.0003, critic_lr=0.0003, q_ewc_reg=owl_params['q_ewc_reg'],
                            n_fisher_sample=40000,
                            gamma=0.99, q_lr=3e-4, batch_size=256, tau=0.005, adaptive_alpha=True, pool_size=100000)

    else:
        agent = SACAgent(state_dim=3, action_dim=1, max_action=3.0, min_action=-3.0, num_tasks=1, hidden_size=256,
                         actor_lr=0.0003,
                         critic_lr=0.0003, q_ewc_reg=0.0, gamma=0.99, q_lr=3e-4, batch_size=256, tau=0.005,
                         adaptive_alpha=True, pool_size=100000)
