...
class CLActor(BasePolicy):
    ...
    def __init__(
        ...
        self.heads = ["{0}.{1}.{2}".format("mean_layer", a, b) for a, b in
                      itertools.product(range(self.num_tasks), ['weight', 'bias'])] + \
                     ["{0}.{1}.{2}".format("log_std_layer", a, b) for a, b in
                      itertools.product(range(self.num_tasks), ['weight', 'bias'])]
    ...
    def calculate_importance(self, state_all, q, alpha):
        ...
        task_heads = ["{0}.{1}.{2}".format(a, int(self.task_idx), b) for a, b in itertools.product(['mean_layer', 'log_std_layer'], ['weight', 'bias'])]
        for i in range(N):
            s = state_all[i]
            q_ = q[i].view(1, -1)
...
class CLContinuousCritic(BaseModel):
    ...
    def calculate_importance(self, state_all, action_all, q_target):
        ...
        self.set_training_mode(False)
        N = list(state_all.size())[0]
        task_heads = ["{0}.{1}.{2}".format(a, int(self.task_idx), b) for a, b in itertools.product(['q1_last', 'q2_last'], ['weight', 'bias'])]
        for i in range(N):
            s = state_all[i]
            a = action_all[i]
            a = a.view(1,-1)
...
class CustomSACPolicy(BasePolicy):
    ...
    def make_actor(self, features_extractor: Optional[BaseFeaturesExtractor] = None) -> CLActor:
        ...
        actor_kwargs["q_ewc_reg"] = self.q_ewc_reg
    ...
    def make_critic(self, features_extractor: Optional[BaseFeaturesExtractor] = None) -> CLContinuousCritic:
        ...
        critic_kwargs["q_ewc_reg"] = self.q_ewc_reg
