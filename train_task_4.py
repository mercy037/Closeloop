...
from sac_stable import SACAgent, SACAgentOWL
...
def train_agent_model_free_multi_task_car_following(agent, envs, params):
    ...
    #     agent, 'Dec12_13-26-18', '1210_test4_right')
    # n_updates = 0
    # samples_number = 0
    while True:
        time_step = 0
        ...
        i_episode += 1
        if i_episode % 500 == 0:  # Condition to terminate, also let's cache agent and envs for plotting later
            if save_model:
                current_time = datetime.now().strftime('%b%d_%H-%M-%S')
...
def make_env(env_id):
    ...
    if env_id == 1:
        ...
        carla_gym_parser.add_argument('-p', '--carla_port', metavar='P', default=4900, type=int,
                                      help='TCP port to listen to (default: 2000)')
        carla_gym_parser.add_argument('--tm_port', default=15299, type=int,
